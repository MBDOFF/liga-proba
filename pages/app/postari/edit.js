...
export default function EditPostPage() {
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      try {
        ...
        setPostImageUrl(data.post.imageUrl || "");
        setPostName(data.post.name || "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Postarea a fost actualizată cu succes!");
      router.push("/app/postari");
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <Section name="app" className="mt-24" bgColor="bg-white">
        <h1 className="text-3xl font-bold text-gray-800 mt-4">Editează Postarea</h1>
        <h2 className="text-2xl font-semibold text-gray-600 mt-1.5">
          {session.user.email} ({session.user.profile.name})
        </h2>
        <p className="mt-4 text-gray-600">
          Modifică detaliile postării selectate mai jos.
        </p>

        {fetching ? (
          <p className="mt-6 text-gray-600">Se încarcă postarea...</p>
        ) : (
          <form onSubmit={handleSubmit} className="mt-6 w-full max-w-2xl">
            <div className="mb-4">
              <label className="block text-gray-700 font-semibold mb-2" htmlFor="postImageUrl">
            ...
            <div className="mb-4">
              <label className="block text-gray-700 font-semibold mb-2" htmlFor="postName">
                Numele Postării
              </label>
              <input
                type="text"
            ...
            <div className="mb-4">
              <label className="block text-gray-700 font-semibold mb-2" htmlFor="postDescription">
                Descrierea Postării
              </label>
              <textarea
                id="postDescription"
            ...
            >
              {loading ? "Se actualizează..." : "Actualizează Postarea"}
            </button>
