...
export default function NewIdeaPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Ideea a fost adăugată cu succes!");
      router.push("/app");
    } catch (err) {
  ...
  return (
    <>
      <Section name="app" className="mt-24" bgColor="bg-white">
        <h1 className="text-3xl font-bold text-gray-800 mt-4">Adaugă o idee</h1>
        <h2 className="text-2xl font-semibold text-gray-600 mt-1.5">
          {session.user.email} ({session.user.profile.name})
        </h2>
        <p className="mt-4 text-gray-600">
          Completează formularul de mai jos pentru a adăuga o nouă idee de proiect.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 w-full max-w-2xl">
          <div className="mb-4">
            ...
            <input
              ...
              placeholder="Introdu numele ideii tale"
            />
          </div>
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              className="w-full px-4 py-2 border rounded-2xl focus:outline-none focus:ring-2 focus:ring-red-800"
              rows="5"
              placeholder="Descrie ideea ta în detaliu"
            ></textarea>
          </div>
          ...
          >
            {loading ? "Se adaugă..." : "Adaugă Ideea"}
          </button>
          <Link
            href="/app"
