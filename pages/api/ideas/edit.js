...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (result.matchedCount === 0) {
      return res.status(404).json({ message: "Ideea nu a fost găsită" });
    }

    res.status(200).json({ message: "Ideea a fost actualizată cu succes" });
  } catch (err) {
    console.error("Eroare la actualizarea ideii:", err);
